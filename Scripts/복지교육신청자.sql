...
--내가 신청한 복지 조회
SELECT title FROM benefit b , ATTENDEELIST a WHERE a.LISTNUMBER =b.listnumber AND a.empno =2022201027 ;
--복지 신청한 사람들 
...
SELECT LISTNUMBER ,title,MAXATTENDEE ,CRTATTENDEE FROM training WHERE LISTNUMBER =168021;

UPDATE TRAINING SET CRTATTENDEE = CRTATTENDEE + 1 WHERE LISTNUMBER = 1646220824 AND CRTATTENDEE < MAXATTENDEE ;



INSERT INTO ATTENDEELIST (SELECT 1646220824, 2022201027 
FROM dual WHERE NOT exists (SELECT 1 FROM ATTENDEELIST WHERE LISTNUMBER=1646220824 AND EMPNO= 2022201027)) ;
...
--내가 신청한 교육
SELECT * FROM training t, ATTENDEELIST a WHERE t.LISTNUMBER =a.LISTNUMBER  AND empno= 2022201020;